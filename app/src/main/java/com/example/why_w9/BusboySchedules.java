...
public class BusboySchedules extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void addShift(View v) {
        ...
        Spinner startTime = (Spinner)findViewById(R.id.timeStart_b);
        String start = startTime.getSelectedItem().toString();

        //get end time from spinner
        Spinner endTime = (Spinner)findViewById(R.id.timeEnd_b);
        String end = endTime.getSelectedItem().toString();

        EditText busboyName_tv = (EditText) findViewById(R.id.editTextBusboyName);
        String busboyName = busboyName_tv.getText().toString();

        //get day option from spinner
        Spinner day_spin = (Spinner)findViewById(R.id.dayOption_b);
        String dayField = day_spin.getSelectedItem().toString();

        if (busboyName.compareTo(busboyList.get(0)) == 0) {
            ...
            listViewAdapter.updateExpListView(busboyList,newDays,0);
        }
        else if(busboyName.compareTo(busboyList.get(1))==0){
            List<String> newDays = new ArrayList<>();
            ...
            listViewAdapter.updateExpListView(busboyList,newDays,1);
        }
    }
